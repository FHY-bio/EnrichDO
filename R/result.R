...
#'drawGraphViz(enrich,numview=FALSE,pview=FALSE)
drawGraphViz <- function(enrich0=enrich, n=10,labelfontsize=14,numview=TRUE,pview=TRUE) {
  ...
  if(pview==TRUE){
	...
	if(pval!=1){pval<-format(pval,digit=5,scientific=T)}
...
#'demo.data=c(1636,351,102,2932,3077,348,4137,54209,5663,5328,23621,3416,3553)
#'doEnrich(interestGenes=demo.data)
#'drawHeatmap(interestGenes=demo.data,enrich = enrich,gene_n = 10,fontsize_row = 8,readable=TRUE)
...
drawHeatmap<-function(interestGenes,enrich=enrich,DOID_n=10,gene_n=50,fontsize_row=10,readable=TRUE,...){
  ...
  weightmatrix<-t(weight_matrix[-(n+1),names(sort(weight_matrix[n+1,],decreasing = T)[1:m])])
  ...
  pheatmap(weightmatrix,border=F, cluster_cols = FALSE,color = colors,angle_col  = 45,fontsize_row = fontsize_row,...)
